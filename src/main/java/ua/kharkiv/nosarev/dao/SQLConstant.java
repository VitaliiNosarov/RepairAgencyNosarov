...
public final class SQLConstant {
    ...
    public static final String FIND_ORDERS = "SELECT b.id, b.price, b.customer_comment, b.device, b.creating_time, " +
            ...
            "group_concat(Distinct s.service_id, '/', s.name, '/', ru.name SEPARATOR '&') AS services FROM booking AS b " +
            "LEFT JOIN booking_service AS b_s ON b_s.booking_id = b.id " +
            "LEFT JOIN service_en AS s ON s.service_id = b_s.service_id " +
            "LEFT JOIN service_ru AS ru ON ru.service_id = b_s.service_id " +
    ...
    public static final String GET_ALL_SERVICES = "SELECT en.service_id , en.name, ru.name FROM service_en As en " +
            "JOIN service_ru As ru ON en.service_id = ru.service_id";
